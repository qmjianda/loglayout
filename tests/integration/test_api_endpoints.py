...
def test_api_open_file_not_found():
    ...
    response = client.post("/api/open_file", json={
        "file_id": "missing",
        "file_path": "/non/existent/path.log"
    })
...
def test_api_read_processed_lines_empty():
    ...
    response = client.get("/api/read_processed_lines", params={
        "file_id": "invalid",
        "start_line": 0,
        "count": 10
    })
...
def test_api_bookmark_endpoints(temp_log_file):
    ...
    # 2. Toggle bookmark
    response = client.post("/api/toggle_bookmark", json={
        "file_id": file_id,
        "line_index": 2
    })
    assert response.status_code == 200
